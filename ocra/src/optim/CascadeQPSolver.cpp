...
void CascadeQPSolver::addLinearConstraint(LinearConstraint::Ptr constraint, size_t hierarchyLevel)
{
    //increasing _hierarchyLevels size if necessary
    if (hierarchyLevel >= _hierarchyLevels.size())
    {
        for (auto _ : _hierarchyLevels)
        {
            _hierarchyLevels.push_back(HierarchyLevelConstraints::Ptr());
            _hierarchyInput.push_back(HierarchyLevel::Ptr());
        }
    }
    ...
    {
        _hierarchyLevels[hierarchyLevel]->_equalities.push_back(std::move(constraint));
        _hierarchyLevels[hierarchyLevel]->_m += constraint->getDimension();
    }
    else
    {
        _hierarchyLevels[hierarchyLevel]->_inequalities.push_back(std::move(constraint));
...
bool CascadeQPSolver::removeLinearConstraint(LinearConstraint::Ptr constraint, size_t hierarchyLevel)
{
    auto v = &_hierarchyLevels[hierarchyLevel]->_inequalities;
    if (constraint->isEquality())
      v = &_hierarchyLevels[hierarchyLevel]->_equalities;

    auto it = std::find(v->begin(), v->end(), constraint);
    if (it != v->end())
    {
...
void CascadeQPSolver::removeLinearConstraint(LinearConstraint::Ptr constraint)
{
    int i=0;
    for (auto _ : _hierarchyLevels)
