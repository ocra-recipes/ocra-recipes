...
void GaussianProcessTrajectory::initializeTrajectory()
{
...
void GaussianProcessTrajectory::printWaypointData()
{
    int w1 = 12;
    int w2 = 5;
    int w3 = 8;
...
bool GaussianProcessTrajectory::setMeanWaypoints(boolVector& bMeanVec)
{
    bool retval;
    if (bMeanVec.empty()) {
...
bool GaussianProcessTrajectory::setVarianceWaypoints(boolVector& bVarVec)
{
    bool retval;
    if (bVarVec.empty()) {
...
bool GaussianProcessTrajectory::setOptimizationWaypoints(boolVector& bOptVec)
{
    bool retval;
    if (bOptVec.empty()) {
...
bool GaussianProcessTrajectory::setDofToOptimize(std::vector<Eigen::VectorXi>& dofToOptVec)
{
    bool retval;
...
bool GaussianProcessTrajectory::gpParametersAreSet()
{
    bool retVal = true;
...
void GaussianProcessTrajectory::calculateGaussianProcessParameters()
{
...
Eigen::MatrixXd GaussianProcessTrajectory::getDesiredValues(double _time)
{
    /**
...
Eigen::VectorXd GaussianProcessTrajectory::getVariance(double _time)
{
...
void GaussianProcessTrajectory::getDesiredValues(double _time, Eigen::MatrixXd& desiredValues, Eigen::VectorXd& variance)
{
...
Eigen::MatrixXd GaussianProcessTrajectory::getWaypointData()
{
...
Eigen::MatrixXd GaussianProcessTrajectory::getMeanGPData()
{
...
Eigen::MatrixXd GaussianProcessTrajectory::getVarGPData()
{
...
void GaussianProcessTrajectory::addWaypoint(const Eigen::VectorXd newWaypoint, const double waypointTime)
{
    addWaypoint(newWaypoint, waypointTime, Eigen::VectorXi::LinSpaced(nDoF+1, 0, nDoF));
}

void GaussianProcessTrajectory::addWaypoint(const Eigen::VectorXd newWaypoint, const double waypointTime, const Eigen::VectorXi& _dofToOpt, const bool useForMean, const bool useForVar, const bool useForOpt)
{
    ...
    if ( (newWaypoint.rows() == nDoF) && (waypointTime>=0.0) )
    {
        ...
        if (newWaypoint.rows() != nDoF){
            std::cout << "[ERROR] (GaussianProcessTrajectory::addWaypoint): New waypoint is not the right size. Should have dim = " <<nDoF << "x1." << std::endl;
        }
        if (waypointTime<=0.0){
            std::cout << "[ERROR] (GaussianProcessTrajectory::addWaypoint): New waypoint time should be greater than 0.0 seconds." << std::endl;
        }
    }
...
void GaussianProcessTrajectory::removeWaypoint(int index)
{
    ...
    {
        std::cout << "[ERROR] (GaussianProcessTrajectory::addWaypoint): New waypoint time is out of index bounds. Should have fall between 0 and " << nWaypoints-1 << "." << std::endl;
    }

}


Eigen::MatrixXd GaussianProcessTrajectory::getBoptCovarianceMatrix()
{
    ...
    if (boptVariablesSet) {
        ...
        for (int i =0; i<waypoints.cols(); i++)
        {
            if (isOptWaypoint[i])
            {
                for(int j=0; j<dofToOptimize[i].size(); j++)
                {
                    ...
                    {
                        std::cout << "[ERROR] (GaussianProcessTrajectory::getBoptCovarianceMatrix): You are trying to optimize an index which doesn't exist! Ignoring." << std::endl;
                    }
                    indexCounter++;
                }
            }
        ...
        std::cout << "[ERROR] (GaussianProcessTrajectory::getBoptCovarianceMatrix): Bayesian optimization variables have not yet been set. Please call GaussianProcessTrajectory::getBoptVariables(const int extraPointsToAdd) first." << std::endl;
    }
    return  covarianceMatrix;
}

Eigen::VectorXd GaussianProcessTrajectory::getBoptSearchSpaceMinBound()
{
    int optVectorSize = 0;
    for (int i =0; i<waypoints.cols(); i++)
    {
    ...
    if (boptVariablesSet)
    {
        ...
        for (int i =0; i<waypoints.cols(); i++)
        {
            if (isOptWaypoint[i])
            {
                for(int j=0; j<dofToOptimize[i].size(); j++)
                {
                    ...
                    {
                        std::cout << "[ERROR] (GaussianProcessTrajectory::getBoptSearchSpaceMinBound): You are trying to optimize an index which doesn't exist! Ignoring." << std::endl;
                    }
                    indexCounter++;
                }
            }
        ...
        std::cout << "[ERROR] (GaussianProcessTrajectory::getBoptSearchSpaceMinBound): Bayesian optimization variables have not yet been set. Please call GaussianProcessTrajectory::getBoptVariables(const int extraPointsToAdd) first." << std::endl;
    }

    return minBound;
}

Eigen::VectorXd GaussianProcessTrajectory::getBoptSearchSpaceMaxBound()
{
    int optVectorSize = 0;
    for (int i =0; i<waypoints.cols(); i++)
    {
    ...
    if (boptVariablesSet)
    {
        ...
        for (int i =0; i<waypoints.cols(); i++)
        {
            if (isOptWaypoint[i])
            {
                for(int j=0; j<dofToOptimize[i].size(); j++)
                {
                    ...
                    {
                        std::cout << "[ERROR] (GaussianProcessTrajectory::getBoptSearchSpaceMaxBound): You are trying to optimize an index which doesn't exist! Ignoring." << std::endl;
                    }
                    indexCounter++;
                }
            }
        ...
        std::cout << "[ERROR] (GaussianProcessTrajectory::getBoptSearchSpaceMaxBound): Bayesian optimization variables have not yet been set. Please call GaussianProcessTrajectory::getBoptVariables(const int extraPointsToAdd) first." << std::endl;
    }
...
Eigen::VectorXd GaussianProcessTrajectory::getBoptVariables(const int extraPointsToAdd, std::vector<Eigen::VectorXi> dofToOptVec)
{
    int nP;
        ...
        if (isOptWaypoint[i]) {
            for(int j=0;j<dofToOptimize[i].size();j++){
                if (dofToOptimize[i](j)==0) {
                    ...
                    std::cout << "[ERROR] (GaussianProcessTrajectory::getBoptVariables): You are trying to optimize an index which doesn't exist! Ignoring and setting to zero." << std::endl;
                }
                indexCounter++;
...
bool GaussianProcessTrajectory::setBoptVariables(const Eigen::VectorXd& newOptVariables)
{
    int optVectorSize = 0;
    for (int i =0; i<waypoints.cols(); i++)
    {
        ...
        for (int i =0; i<waypoints.cols(); i++)
        {
            if (isOptWaypoint[i])
            {
                for(int j=0; j<dofToOptimize[i].size(); j++)
                {
                    ...
                    {
                        std::cout << "[ERROR] (GaussianProcessTrajectory::getBoptVariables): You are trying to change an index which doesn't exist! Ignoring." << std::endl;
                    }
                    indexCounter++;
                }
            }
...
// smlt::bopt_Parameters GaussianProcessTrajectory::getBoptParameters()
// {
...
void GaussianProcessTrajectory::precalculateTrajectory(Eigen::MatrixXd& _traj, Eigen::MatrixXd& _variance, Eigen::VectorXd& _timeline, const double DT)
{
...
void GaussianProcessTrajectory::saveTrajectoryToFile(const std::string dirPath)
{
...
void GaussianProcessTrajectory::saveWaypointDataToFile(const std::string dirPath)
{
    smlt::checkAndCreateDirectory(dirPath);
