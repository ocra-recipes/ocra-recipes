/** @file ObjQLD.h
  * @brief Declaration file of the ObjQLD class.
  * This class should be merged with ocra::ObjQLD. It has been created to
  * have a cml-free solver
  *
...
#ifndef _MPCW_OBJ_QLD_EIGEN_H_
#define _MPCW_OBJ_QLD_EIGEN_H_
...
namespace ocra
{
  ...
  public:
    ...
    // ------------------------ public interface --------------------------------
  public:
    int solve(Map<MatrixXd>& C, const Map<VectorXd>& d, const Map<MatrixXd>& A, const Map<VectorXd>& b, int me,
              Map<VectorXd>& x, const Map<VectorXd>& xl, const Map<VectorXd>& xu, bool factorizedC = false); //< if factorizedC == true, the upper triangle factor of choleski factorization of C must be given instead of C

    const   VectorXd& getLagrangeMultipliers(void) const;
    double  getSizeFactor(void) const;
    void    setSizeFactor(double f);
    double  getEps(void) const {return _eps;}
  ...
  protected:
    ...
    // ------------------------ private methods ---------------------------------
  private:
    //Matrices begins at 0 and are column major
    static int ql0001_(int *m,int *me,int *mmax,int *n,int *nmax,int *mnn,
                      ...
                      double *eps1);
    static int ql0002_(int *n,int *m,int *meq,int *mmax,
  ...
  private:
    inline static int abs(int x) {if (x >= 0) return x; else return -x;}
    inline static double abs(double x) {if (x >= 0.) return x; else return -x;}
    inline static int min(int a, int b) {if (a <= b) return a; else return b;}
    inline static int max(int a, int b) {if (a >= b) return a; else return b;}
    inline static double min(double a, double b) {if (a <= b) return a; else return b;}
    inline static double max(double a, double b) {if (a >= b) return a; else return b;}
  ...
  private:
    ...
    int       _liwar;
    int       _mnn;
    double    _sizeFactor;          //< lwar (resp. liwar) is equal to the minimum possible value of lwar (resp. liwar) times sizeFactor
    double    _eps;
    int*      _iwar;                //< interger working array
    int       _iwarCapacity;
    VectorXd  _war;                 //< working array
    VectorXd  _u;                   //< lagrange multipliers
