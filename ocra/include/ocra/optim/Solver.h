...
protected:
    ...
    //@}
       ...
       *  @throw This method will throw a runtime_error if one attempts to add an already added objective that does not exist in the problem.
       */
    void internalAddObjective(const GenericObjective& objective);
       ...
       *  @throw This method will throw a runtime_error if one attempts to add an already added constraint that does not exist in the problem.
       */
    void internalAddConstraint(const GenericConstraint& constraint);
       ...
       *  @throw This method will throw a runtime_error if one attempts to remove an already removed objective that does not exist in the problem.
       */
    void internalRemoveObjective(const GenericObjective& objective);
       ...
       *  @throw This method will throw a runtime_error if one attempts to remove an already removed constraint that does not exist in the problem.
       */
    void internalRemoveConstraint(const GenericConstraint& constraint);

      /**
       *  Callback method to be invoked when the function of a constraint indicates an EVT_RESIZE, or the
       *   constraint itself triggers an EVT_CHANGE_BOUNDS_NUMBER event.
       *
  ...
  private:
      /**
       *  A flag on the problem variable validity.
       */
    bool                                  _isVariableUpToDate;
      /**
       *  A composite variable built by merging the functions' variables and storing their mappings.
       */
    MergedVariable                        _problemVariable;
      /**
       *  Set of objectives
       */
    std::vector<const GenericObjective*>  _objectives;
      /**
       *  Set of constraints
       */
    std::vector<const GenericConstraint*> _constraints;
      /**
       *  Stores the state of the solver before doSolve if memory level > 0
       */
    boost::circular_buffer<std::string>   _memory;
      /**
       *  if not empty, file where the saved data will be dumped if an error occurs
       */
    std::string                           _autodumpFile;
    bool                                  _autodump;
  };
}
