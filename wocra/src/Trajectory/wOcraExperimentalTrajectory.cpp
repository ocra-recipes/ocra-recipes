...
void wOcraExperimentalTrajectory::calculateVarianceParameters()
{
...
Eigen::MatrixXd wOcraExperimentalTrajectory::getWaypointData()
{
...
Eigen::MatrixXd wOcraExperimentalTrajectory::kernelFunction(double m)
{
...
Eigen::MatrixXd wOcraExperimentalTrajectory::kernelFunction(Eigen::VectorXd& evalVec)
{
...
Eigen::VectorXd wOcraExperimentalTrajectory::rbfnKernelFunction(double m)
{
        return ((-(m - kernelCenters.array()).array().square()).array() / kernelLengthParameter).array().exp();
}

Eigen::MatrixXd wOcraExperimentalTrajectory::rbfnKernelFunction(Eigen::VectorXd& evalVec)
{
    int sizeVec = evalVec.rows();
    int nC = kernelCenters.rows();
...
void wOcraExperimentalTrajectory::calculateRbfnWeights()
{
    int nC = kernelCenters.rows();
...
Eigen::VectorXd wOcraExperimentalTrajectory::getRbfnOutput(double m)
{
    Eigen::VectorXd phi = rbfnKernelFunction(m);
    return rbfnWeights * phi;
}

Eigen::MatrixXd wOcraExperimentalTrajectory::getRbfnOutput(Eigen::VectorXd& evalVec)
{
...
void wOcraExperimentalTrajectory::precalculateTrajectory(double DT, Eigen::MatrixXd& _path, Eigen::VectorXd& _timeline)
{
...
Eigen::VectorXd wOcraExperimentalTrajectory::getVariance(double time)
{
...
void wOcraExperimentalTrajectory::getDesiredValues(double time, Eigen::MatrixXd& desiredValues, Eigen::VectorXd& variance)
{
...
void wOcraExperimentalTrajectory::initializeTrajectory()
{
...
double wOcraExperimentalTrajectory::getMaxVariance()
{
    return maxCovariance.maxCoeff();
}
void wOcraExperimentalTrajectory::reinitialize()
{
...
void wOcraExperimentalTrajectory::addNewWaypoint(Eigen::VectorXd newWaypoint, double waypointTime)
{
    ...
    if ( (newWaypoint.rows() == nDoF) && (waypointTime>=0.0) && (waypointTime<=kernelCenters.maxCoeff()) )
    {
        ...
        if (newWaypoint.rows() != nDoF){
            std::cout << "[ERROR] (wOcraExperimentalTrajectory::addNewWaypoint): New waypoint is not the right size. Should have dim = " <<nDoF << "x1." << std::endl;
        }
        if ((waypointTime<=0.0) || (waypointTime>=kernelCenters.maxCoeff())){
            std::cout << "[ERROR] (wOcraExperimentalTrajectory::addNewWaypoint): New waypoint time is out of time bounds. Should have fall between 0.0 and " << kernelCenters.maxCoeff() << " seconds." << std::endl;
        }
    }
...
void wOcraExperimentalTrajectory::removeYoungestWaypoint()
{
    removeWaypoint(youngestWaypointIndex);
}

void wOcraExperimentalTrajectory::removeWaypoint(int index)
{
    ...
    if ( (index>=0) && (index<nWaypoints) )
    {
        ...
        std::cout << "[ERROR] (wOcraExperimentalTrajectory::addNewWaypoint): New waypoint time is out of index bounds. Should have fall between 0 and " << nWaypoints-1 << "." << std::endl;
    }
...
Eigen::MatrixXd wOcraExperimentalTrajectory::getRbfnKernelCurves()
{
    int nSteps = 1000;
    int nC = kernelCenters.rows();
